 ...
 * @param {string} [options.comparisonType='unordered'] - 'ordered' or 'unordered' for arrays
 ...
 *
 * // Global ellipsis for flexible object matching (parser adds '...': '...' automatically)
 * areObjectsEqual({ a: 1, '...': '...' }, { a: 1, b: 2, c: 3 }) // true (allows extra fields)
 */
 ...
 * compareArrays([{a:1}, {b:2}], [{b:2}, {a:1}], {comparisonType: 'unordered'}) // true
 */
function compareArrays(arrayA, arrayB, options) {
  ...
  const comparisonType = options.comparisonType || 'unordered';
  const isUnordered = comparisonType === 'unordered';
  const isOrdered = comparisonType === 'ordered';
